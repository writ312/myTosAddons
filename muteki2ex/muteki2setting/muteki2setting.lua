...
function MUTEKI2_CHANGE_BUFFID(list,control,oldIDbuffid,argNum)
  ...
  MUTEKI2_CREATE_SETTING_FRAME()
  -- MUTEKI2_UPDATE_LIST_VIEW(list,newID)
end
...
function MUTEKI2_CHANGE_COLORTONE(list,control,buffid,argNum)
  ...
  local oldColor = obj[buffid].color
  ...
  if obj.circleIcon then
    g.circle[buffid] = MUTEKI2_INIT_CIRCLE(frame,buff)
      ...
      MUTEKI2_ADD_GAUGE_BUFF(MUTEKI2_GET_BUFF(tonumber(buffid)),g.gauge[buffid])
  end
...
function MUTEKI2_ADD_BUFFID(frame,control,buffName,buffid)
  if not g.settings.buffList[tostring(buffid)] then
    g.settings.buffList[tostring(buffid)] = {color = defaultColor}
    ui.SysMsg(string.format("MUTEKI2に%sを追加しました.",GetClassByType('Buff',buffid)))
    MUTEKI2_ADD_GAUGE_BUFF(MUTEKI2_GET_BUFF(buffid),MUTEKI2_GET_GAUGE_BUFF(buffid))
    MUTEKI2_CREATE_SETTING_FRAME()
  end
end
